...
> 📐 Design Tokens（色彩、字體、間距、圓角、陰影、動畫）見 [`01-6-UI_UX_Design.md` §2](./01-6-UI_UX_Design.md#2-design-tokens)
...
graph TB
    ...
    subgraph ExternalLLM["外部 LLM 服務"]
        OpenAI["OpenAI API<br/>（預設）"]
    ...
    DataExtractor -->|API Call| OpenAI
    DataExtractor -->|API Call| Gemini
    DataExtractor -->|API Call| Anthropic
    DataExtractor -->|API Call| XAI
    PersonaEngine -->|API Call| OpenAI
    PersonaEngine -->|API Call| Gemini
    PersonaEngine -->|API Call| Anthropic
    PersonaEngine -->|API Call| XAI
...
erDiagram
    ...
    USERS {
        ...
        enum ai_engine "gemini/openai/anthropic/xai default openai"
...
> 📐 類別色彩表見 [`01-6-UI_UX_Design.md` §3.2 統計頁](./01-6-UI_UX_Design.md#32-統計頁-stats)
...
# 模型過濾（可選，正則表達式）
GEMINI_MODEL_INCLUDE=
GEMINI_MODEL_EXCLUDE=
OPENAI_MODEL_INCLUDE=
OPENAI_MODEL_EXCLUDE=
ANTHROPIC_MODEL_INCLUDE=
ANTHROPIC_MODEL_EXCLUDE=
XAI_MODEL_INCLUDE=
XAI_MODEL_EXCLUDE=
...
| v1.5 | 2026-03-27 | M7 Release 規格同步：預設引擎改為 OpenAI（ER 圖 + SQL Schema + 架構圖）、架構圖新增 Anthropic/xAI、環境變數補充 MODEL_INCLUDE/EXCLUDE、§6 子章節編號修正 |
| v1.6 | 2026-04-09 | 規格文件改名對齊新版 vibe-sdlc-spec：UI/UX 設計文件由 `01-4-UI_UX_Design.md` 改為 `01-6-UI_UX_Design.md`，更新文中所有 UI/UX 設計引用連結 |
