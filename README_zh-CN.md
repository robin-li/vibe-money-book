...
[**Live Demo**](#-live-demo) · [功能特色](#-功能特色) · [快速开始](#-快速开始) · [截图展示](#-截图展示)
...
> 「午餐吃拉面 180 元」「在阿猫阿狗帮狗子买狗粮 1500」「上周五和同事聚餐 AA 制，我付了 650」

别再手动输入金额、选类别、挑日期。**按住说话，AI 全部搞定。**

- 🗣️ **语音 + 文字双输入**：整合 Web Speech API，按住说话或直接打字
- 🧠 **AI 智慧解析**：LLM 自动萃取金额、类别、商家、日期，一张卡片搞定
- 📂 **自动分类**：遇到新类别？AI 主动建议，一键新增
- 📅 **语义日期辨识**：说「上周五」「白色情人节」「38 节」都能精准识别

### 🔍 用说的查账 — 语音 / 语义查询

> 「这个月花在阿猫阿狗多少钱？」「最近毛小孩的开销」「吃的方面花了多少」

不用记分类名称，不用手动筛选，**开口问就有答案。**

- 🎯 **两阶段 AI 分析**：时间范围解析 → 交易智慧匹配
- 💬 **人设风格反馈**：AI 教练用你选的风格（毒舌 / 温柔 / 情勒）总结花费
- 🔗 **查询 + 筛选互斥**：手动筛选器（日期／类别）与 AI 查询自动切换，「清除筛选」一键重置

### 🤖 个性化 AI 教练 — 你的专属财务评论员

每一笔消费，AI 教练都会即时点评。三种人设，选你喜欢的：
...
通过「AI 指示」功能，自定义专属的分类逻辑，AI 会优先遵循你的规则：
...
搭配「自定义类别」功能，建立你自己的分类体系。同名类别还能同时存在于收入和支出！

### 🌍 四语言完整支持

🇹🇼 繁體中文 · 🇺🇸 English · 🇨🇳 简体中文 · 🇻🇳 Tiếng Việt

- UI、错误消息、AI 反馈全部跟随语言设置
- 登录前就能切换语言，语音识别自动跟随
- 三层持久化：DB → localStorage → 浏览器侦测
