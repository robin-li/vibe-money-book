...
graph TB
    ...
    subgraph LLMServices["外部 LLM 服務"]
    ...
    SPA --> PWA
    SPA --> I18nFE
    SPA -->|API 請求<br/>Authorization + Accept-Language<br/>+ X-LLM-API-Key| Router
    ...
    Services --> I18nBE
    LLMAbstraction -->|API Call| Gemini
    LLMAbstraction -.->|備選| OpenAI
...
| **LLM 服務層** | 自然語言解析、情緒回饋生成、語義查詢 | 雙引擎抽象，使用者自帶 API Key |
...
| **OpenAI SDK** | — | LLM 客戶端 | 官方 SDK、型別完整、串流支援 |
| **Google Generative AI SDK** | — | LLM 客戶端 | 官方 SDK、Gemini 模型存取 |
...
| `ai_engine` | VARCHAR(20) | NOT NULL, DEFAULT 'gemini' | LLM 引擎偏好：gemini / openai |
...
系統採用 LLM 雙引擎設計，透過抽象層統一介面，支援 Gemini 與 OpenAI 動態切換。

```mermaid
graph LR
    subgraph Engines["LLM 引擎"]
    ...
    QE --> LP
    LP --> LR
    LR -->|user.ai_engine = gemini| GP
    LR -.->|user.ai_engine = openai| OP
```
...
```typescript
// llmRouter.ts
function getLLMProvider(aiEngine: 'gemini' | 'openai'): LLMProvider {
  switch (aiEngine) {
    case 'gemini':
...
| 版本 | 日期 | 修訂內容 |
|------|------|---------|
| v1.0 | 2026-03-22 | 初版定稿。從 SRD v1.2 抽離設計相關章節，新增：系統架構總覽、技術棧選型決策理由、前後端架構職責說明、數據模型 ER 圖與索引設計、LLM 抽象層與 Prompt 模板結構、國際化架構設計、ADR 設計決策記錄（4 項）、錯誤處理策略（含 LLM Fallback）、快取與效能設計、安全設計（含認證時序圖與限流方案） |
