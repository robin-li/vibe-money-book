...
| 🥧 Phân tích chi tiêu | Biểu đồ tròn + biểu đồ cột tương tác, nhấn danh mục xem chi tiết, thu nhập chi tiêu thống kê riêng |
| 🔄 Bốn AI Engine | Gemini / OpenAI / Anthropic / xAI tự do chuyển đổi, quản lý API Key riêng từng nhà cung cấp |
...
| **AI / LLM** | OpenAI (mặc định) · Google Gemini · Anthropic Claude · xAI Grok — Bốn engine tự do chuyển đổi |
| **Testing** | Vitest · Testing Library · Supertest · Playwright |
| **Triển khai** | Docker · Docker Compose · Cloudflare Tunnel · PostgreSQL |
...
| Model | Đặc điểm | Giá (per 1M tokens) |
|------|------|---------------------|
| `claude-haiku-4-5-20251001` ⭐ | Nhanh nhất, lý tưởng cho tác vụ hàng ngày (khuyên dùng) | $1.00 / $5.00 |
| `claude-sonnet-4-6` | Cân bằng tốc độ và trí tuệ tốt nhất | $3.00 / $15.00 |
| `claude-opus-4-6` | Thông minh nhất, tốt nhất cho agent và lập trình | $5.00 / $25.00 |

#### xAI Grok

| Model | Đặc điểm | Giá (per 1M tokens) |
|------|------|---------------------|
| `grok-3-mini-fast` ⭐ | Nhanh nhất, tiết kiệm nhất (khuyên dùng) | — |
| `grok-3-mini` | Cân bằng, suy luận tốt | — |
| `grok-3` | Model mạnh nhất của xAI | — |
...
|------|------|--------|
| `DATABASE_URL` | Chuỗi kết nối cơ sở dữ liệu | `postgresql://vibe:vibe_password@postgres:5432/vibe_money_book` (PostgreSQL) |
...
| `ANTHROPIC_MODEL` | Tên model Anthropic | `claude-haiku-4-5-20251001` |
| `XAI_MODEL` | Tên model xAI | `grok-3-mini-fast` |
| `{PROVIDER}_MODEL_INCLUDE` | Regex lọc model bao gồm (vd: `GEMINI_MODEL_INCLUDE`) | — |
| `{PROVIDER}_MODEL_EXCLUDE` | Regex lọc model loại trừ | — |
...
| | GET | `/ai/providers` | Lấy danh sách nhà cung cấp AI được hỗ trợ |
| | GET | `/ai/models` | Lấy danh sách model khả dụng của nhà cung cấp |
